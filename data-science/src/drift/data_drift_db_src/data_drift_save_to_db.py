...
for filename in arr:
    if ".csv" in filename:
        ...
        with open(os.path.join(tansformed_data_path, filename), "r") as handle:
            input_df = pd.read_csv((Path(tansformed_data_path) / filename)).dropna()
...
for col in columns:
    ...
    if col in categorical_columns:
        ...
        # statistical test:
        observations = contingency_table(reference[col], current[col])
    ...
    # print("Ref")
    # print(np.where(np.isnan(ref_arr)))
    # print(np.where(np.isinf(ref_arr)))

    # print("Current")
    # print(np.where(np.isnan(curr_arr)))
    # print(np.where(np.isinf(curr_arr)))
